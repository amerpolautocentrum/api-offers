...
function populateDynamicFilters(offers, selectedBrand = null) {
  ...
  offers.forEach(d => {
    ...
    if (!selectedBrand || make === selectedBrand.toLowerCase()) {
      if (selectedBrand && make === selectedBrand.toLowerCase()) {
        debugModels.push(d);
      }
  ...
  if (selectedBrand) {
    ...
    if (debugModels.length > 0) {
      console.log(`📦 Przykładowa oferta marki ${selectedBrand}:`, debugModels[0]);
    } else {
      console.warn(`⚠️ Brak ofert dla marki ${selectedBrand}`);
    }
  }
